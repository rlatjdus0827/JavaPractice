...
//						mem.setMem_name(rs.getString("mem_name"));
	...
	public int memUpdate(MemDTO mem) {
		int result = 0;
		String sql = " update members set MEM_NAME= ?, MEM_EMAIL= ?, MEM_PHONE= ? " + " where MEM_ID= ?";
		conn = DBUtil.dbConnection();
		try {
			pst = conn.prepareStatement(sql);
			pst.setString(4, mem.getMem_id());
			pst.setString(1, mem.getMem_name());
			pst.setString(2, mem.getMem_email());
			pst.setString(3, mem.getMem_phone());
			result = pst.executeUpdate();
	...
	public int trnUpdate(TrainDTO trn) {
		int result = 0;
		String sql = "update trainers set TRN_NAME=?, TRN_EMAIL=?, TRN_PHONE=? where TRN_ID= ? ";
		conn = DBUtil.dbConnection();
		try {
			pst = conn.prepareStatement(sql);
			pst.setString(4, trn.getTrn_id());
			pst.setString(1, trn.getTrn_name());
			pst.setString(2, trn.getTrn_email());
			pst.setString(3, trn.getTrn_phone());
			result = pst.executeUpdate();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
